...
DiDe <- function(inputdata, varnames, control_group = "all", base_event=-1, min_event=NULL, max_event=NULL, return_data=FALSE, return_ATTs_only=TRUE, parallel_cores=1){
  ...
  get_cohort_results <- function(cc){
    ...
    for(event_postperiod in event_periods){
      res = DiDge_hb(inputdata, cohort_time = cc, event_postperiod = event_postperiod, base_event = base_event,
...
#'
#' @export
DiD_hb <- function(inputdata, varnames, control_group = "all", base_event=-1, min_event=NULL, max_event=NULL, Esets=NULL, return_ATTs_only=TRUE, parallel_cores=1){
