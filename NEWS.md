...
- Added `effect_fit`: `"separate"` (one fit per component on its clean cells), `"joint"` (a model for the sequential marginal effect of every event, fit on every cell; the cell is the telescoping sum of its active marginal effects, so no additivity is assumed), `"ordered"` (the same for the k-th occurrence of one event kind, with a check that the dates increase), and `"state"` (the cell as a function of its state: number of active events, parity, time since the last event; the effect of event k is the change of state, which covers an on-and-off treatment)
...
- Improved input validation: errors on negative `anticipation`/`anticipation2`, time-varying weights, and `balanced_event_time` exceeding data range
- Restored `parglm` dependency for multi-threaded propensity score estimation; thread count automatically matched to `getDTthreads()` (or 1 when `parallel = TRUE`)
- Various bug fixes and robustness improvements for double DiD aggregation and weight handling
