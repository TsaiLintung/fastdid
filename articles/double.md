# Introduction to Double DiD

Here we show how to deal with confounding events, see [Tsai
(2024)](https://arxiv.org/abs/2409.05184) for the full treatment. We
...
Using the default estimator, the estimates (black line) is biased from
...
![](double_files/figure-html/naive-1.png) Diagnostics can be obtained by
using `time >= G2` as outcome. As we set the effect of the confounding
event at 10 constantly, we can see that the bias of the default
...
![](double_files/figure-html/diag-1.png)

Using the double did estimator with `cohortvar2`, the estimator recovers
the ground truth.
...
![](double_files/figure-html/double-1.png)

Double DiD also allow for two additional aggregation scheme:
group-group-time (“group_group_time”) and dynamic-staggered
