...
#' @param effect_fit character, how a formula is fit. `"separate"` fits the modeled component on its clean cells. `"joint"` models the sequential marginal effect of every event, in the order of the cohort columns, and fits on every cell; the cell is the sum of the marginal effects of its active events, so no additivity is assumed. `"ordered"` is the same fit for the k-th occurrence of one event kind, and checks that the cohort dates increase. `"state"` models the cell as a function of its state (the number of active events `nact`, its parity `status`, and the time since the last event `e`), and reports the marginal effect of event k as the state after k events minus the state after k-1 events; the dates must increase.
#'
